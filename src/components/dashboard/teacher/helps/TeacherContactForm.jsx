...
const TeacherContactForm = () => {
  ...
  return (
    <form className='teacher-helpcenter-contact-form' onSubmit={handleSubmit}>
      <div className='teacher-helpcenter-form-row'>
        <div className='teacher-helpcenter-form-group'>
          <label
            htmlFor='supportType'
            className='teacher-helpcenter-form-label'
          >
            Support Type
          </label>
          <IconSelect
        ...
        <div className='teacher-helpcenter-form-group'>
          <label htmlFor='subject' className='teacher-helpcenter-form-label'>
            Subject
          </label>
          <IconInput
      ...
      <div className='teacher-helpcenter-form-group'>
        <label htmlFor='message' className='teacher-helpcenter-form-label'>
          Message
        </label>
        <IconTextarea
      ...
      <div className='teacher-helpcenter-form-group'>
        <label className='teacher-helpcenter-form-label'>
          Attachment (Max 5 files, 5MB each)
        </label>
        <div
          className='teacher-helpcenter-form-attachments-dropzone'
        ...
        >
          ...
          <img
            src={attachIcon}
            alt='Upload files'
            className='teacher-helpcenter-upload-icon'
          />
          <p className='teacher-helpcenter-upload-text'>
            Drop files here or click to upload
          </p>
          <p className='teacher-helpcenter-upload-formats'>
            JPEG, PNG, PDF and MP4 formats up to 50MB.
          </p>

          {/* Display uploaded files */}
          {files.length > 0 && (
            <div className='teacher-helpcenter-uploaded-files'>
              <p className='teacher-helpcenter-uploaded-files-title'>
                Selected files:
              </p>
              <ul className='teacher-helpcenter-uploaded-files-list'>
                {files.map((file, index) => (
                  <li
                    key={index}
                    className='teacher-helpcenter-uploaded-file-item'
                  >
      ...
      <div className='teacher-helpcenter-form-actions'>
        <button
          type='button'
          className='teacher-helpcenter-form-cancel'
          onClick={() => {
            setFormData({
              supportType: 'technical support',
              subject: '',
        ...
        <button
          type='submit'
          className='teacher-helpcenter-form-submit'
      ...
      {submitStatus.success && (
        <div className='teacher-helpcenter-success-message'>
          Your message has been sent successfully! We'll get back to you soon.
        </div>
      )}

      {submitStatus.error && (
        <div className='teacher-helpcenter-error-message'>
          {submitStatus.error}
        </div>
      )}
    </form>
